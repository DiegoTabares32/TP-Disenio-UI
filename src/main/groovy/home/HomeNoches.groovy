...
class HomeNoches {
	...
	def init(){
		...
		noches << new Noche("23/05/2013", homeButacas.get(6), homeBandas.get(1))
		noches << new Noche("4/10/2013", homeButacas.get(7), homeBandas.get(1))
	}
	...
	def searchReservadas(fecha, contrasenia){
		def nochesR = this.noches.findAll{ it -> it.fecha == fecha}
		def butacas = [] as Set
		if(nochesR != null){
			for(noche in nochesR){
